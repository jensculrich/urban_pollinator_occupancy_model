// dynocc model 8 - dynocc 5 but with
// a moderator of whether the site was pre-established
...
data {
  ...
  int<lower=0, upper=1> preestablished[n_sites]; 
...
parameters {
  ...
  real psi1_preestablished; // was the site meadow preestablished?
    ...
    for(j in 1:n_sites){    // loop across all sites
      for(k in 1:n_years_minus1){ // loop across all years
        ...
        psi1[i,j] = inv_logit( // probability (0-1) of occurrence in year 1 is equal to..
          ...
          psi1_preestablished * preestablished[j] +
...
model {
  ...
  psi1_preestablished ~ normal(0, 2);
