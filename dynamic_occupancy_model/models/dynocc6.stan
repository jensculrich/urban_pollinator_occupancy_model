// dynocc model 5 - dynocc 6 but with 
// effects of herb abundance AND diversity
...
data {
  ...
  real herbaceous_diversity_scaled[n_sites, n_years]; // herbaceous flower abundance in quadrats
...
parameters {
  ...
  real psi1_herbaceous_diversity; // effect of herbaceous flowers
  ...
  real psi1_interaction_3; // herb diversity and spec.
    ...
    for(j in 1:n_sites){    // loop across all sites
      for(k in 1:n_years_minus1){ // loop across all years
        ...
        psi1[i,j] = inv_logit( // probability (0-1) of occurrence in year 1 is equal to..
          ...
          psi1_herbaceous_diversity * herbaceous_diversity_scaled[j,1] + 
          (psi1_interaction_3 * d[i] * herbaceous_diversity_scaled[j,1]) 
          ); // end phi[j,k]
        ...
        gamma[i,j,k] = inv_logit( // probability (0-1) of colonization is equal to..
          ...
          gamma_herbaceous_diversity * herbaceous_diversity_scaled[j,k+1] + 
          (gamma_interaction_3 * d[i] * herbaceous_diversity_scaled[j,k+1]) 
          ); // end phi[j,k]
        ...
        phi[i,j,k] = inv_logit( // probability (0-1) of persistence is equal to..
          ...
          phi_herbaceous_diversity * herbaceous_diversity_scaled[j,k] + 
          (phi_interaction_3 * d[i] * herbaceous_diversity_scaled[j,k]) 
          ); // end phi[j,k]
...
model {
  ...
  psi1_herbaceous_diversity ~ normal(0, 2);
  ...
  gamma_herbaceous_diversity ~ normal(0, 2);
  ...
  gamma_interaction_3 ~ normal(0, 2);
  ...
  phi_herbaceous_diversity ~ normal(0, 2);
