...
data {
  ...
  int<lower=0> n_years;
  int<lower=0> n_years_minus1; // for variable annual transitions
...
transformed parameters {
  ...
  for(i in 1:n_species){
    for(j in 1:n_sites){    // loop across all sites
      for(k in 1:n_years_minus1){ // loop across all intervals
    ...
    for(j in 1:n_sites){
      for(k in 1:n_years){
        ...
        if(k < 2){ // define initial state
          ...
          // k - 1 indicates that occurrence this year is due to persistence / colonization following last year
...
model {
  ...
  phi_year ~ normal(0,1); // year effects
  // prior on year effects?
