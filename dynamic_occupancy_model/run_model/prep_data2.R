...
process_raw_data <- function(min_unique_detections, filter_nonnative_woody) {
  ...
  df <- mydata_filtered %>%
    ...
    # filter china creek
    filter(!SITE %in% c("china creek", "memorial south")) %>%
  ...
  HABITAT_CATEGORY <- c(0,0,1,0,0,1,1, # 9 mowed sites # and 7 meadow sites
                        0,1,1,1,0,0,0,1,0) 
  ...
  site_visits <- as.data.frame(cbind(
    rep(site_vector, each=3*6), rep(1:6, times=16*3), rep(1:3, each = 6, times=16), 
    rep("no flowering species", 288), 
    rep(0, 288))) %>%
    rename("SITE" = "V1",
           "SAMPLING_ROUND" = "V2",
           "YEAR" = "V3",
