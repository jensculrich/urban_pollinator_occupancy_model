...
p_min = -0.5 # set these to zero if you want no year heterogeneity
p_max = 0.5
...
simulate_data <- function(
  ...
  p_year <- vector(length = n_years)
  for(k in 1:length(p_year)){
    p_year[k] <- runif(1, min=p_min, max=p_max)
  }
    ...
    for(j in 1:n_sites){
      for(k in 1:n_years_minus1){
        ...
        logit_psi1[i,j] = 
          ...
          (psi1_interaction_1 * d_scaled[i] * habitat_type[j]) +
        ...
        logit_gamma[i,j,k] = # gamma for transition (starting for between years 1 and 2)
          ...
          gamma_herbaceous_flowers * habitat_type[j] + # flowers starting with flowers year 2 for transition 1
          gamma_specialization * d_scaled[i] +
          (gamma_interaction_1 * d_scaled[i] * habitat_type[j]) +
        ...
        logit_phi[i,j,k] = 
          ...
          (phi_interaction_1 * d_scaled[i] * habitat_type[j]) +
...
p_year <- my_simulated_data$p_year
