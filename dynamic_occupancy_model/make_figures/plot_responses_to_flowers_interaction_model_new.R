...
herb_df_spec <- as.data.frame(cbind(original_herb, 
                                    ...
                                    as.vector(criSpec_50[2,,1,1:12]), 
                                    as.vector(criSpec_50[1,,1,1:12]),
                                    as.vector(criSpec_50[3,,1,1:12]))) %>%
...
woody_df_spec <- as.data.frame(cbind(original_woody, 
                                    rep(1:n_bins, each=pred_length),
                                    as.vector(criSpec_50[2,,2,1:12]), 
                                    as.vector(criSpec_50[1,,2,1:12]),
                                    as.vector(criSpec_50[3,,2,1:12]))) %>%
  mutate(specialization_bin = as.factor(V2)) %>%
  rename("mean" = "V3",
         "lower50" = "V4",
         "upper50" = "V5")
